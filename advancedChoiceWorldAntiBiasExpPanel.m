classdef advancedChoiceWorldAntiBiasExpPanel < eui.ExpPanel
  ...
  methods
    function obj = advancedChoiceWorldAntiBiasExpPanel(parent, ref, params, logEntry)
    ...
    function expUpdate(obj, rig, evt)
      ...
      switch type
        case 'signals' %queue signal updates
          ...
          if any(idx)
              obj.Block.trial(i).contrast = updates(idx).value; 
          end
          ...
          if any(idx)
            ...
            obj.Block.trial(i).responseTimes = t;
            lastidx = obj.InputSensorPosCount + 1;
            obj.InputSensorPosCount = lastidx;
...
%           idx = strcmp('events.responseTimes', {updates.name});
%           if any(idx)
%               obj.Block.trial(i).responseTimes = updates(idx).value; 
%           end
          
%           idx = strcmp('events.stimulusOnTimes', {updates.name});
%           if any(idx); obj.Block.trial(i).stimulusOnTimes = updates(idx).value; end
          
          %LMN
          idx = strcmp('events.stimulusOn', {updates.name});
          if any(idx)
              t = (24*3600*datenum(updates(idx).timestamp))-(24*3600*obj.StartedDateTime);
              obj.Block.trial(i).stimulusOnTimes = t;
          end
