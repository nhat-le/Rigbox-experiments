function stimPresentationAndReward(t, evts, p, vs, in, out, audio)
%% advancedChoiceWorld
...
correctResponse = cond(contrastLeft > contrastRight, -1,... % contrast left
    ...
    (contrastLeft == contrastRight) & (rndDraw > 0), 1); % equal contrast (baited)
% feedback = true; %correctResponse == response;
% Only update the feedback signal at the time of the threshold being crossed
feedback = at(true, threshold); 
feedback = merge(feedback > 0, responseTimeOver); % reward when: make a correct movement OR response time is over
